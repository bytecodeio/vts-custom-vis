...
export function externalTooltipHandler(
  ...
  const pivotLabelStyles: Partial<CSSStyleDeclaration> = {
  ...
  const labelColorElement = createElement("span", pivotLabelStyles);
  ...
  pivotLabelElement.appendChild(labelColorElement);
  pivotLabelElement.appendChild(labelTextElement);
  ...
  if (hasPivot) {
    tooltipElement.appendChild(pivotLabelElement);
  }
